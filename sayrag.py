...
def generate_reflections(reflection_model: LLM, 
    ...
    outputs = reflection_model.generate(
        queries,
...
class SayRAGPipeline(SequentialPipeline):
    ...
    def get_prompt(self, question, retrieval_result, reflection_retrieval_result,
                       reflection_result, min_naive_confidence,
                       topk:int, augumented_num:int):
            
            if not reflection_result['available']:
                prompt = self.prompt_template.get_string(question=question, retrieval_result=retrieval_result)
                return prompt

            if reflection_result['confidence'] >= min_naive_confidence:
    ...
    def run(self, dataset, do_eval=True, pred_process_fun=None,
            ...
            min_naive_confidence:int=100, 
            add_query_to_reflection:bool=True):
        
        input_query = dataset.question
        
        assert os.path.isfile(reflection_path)
        reflection_results = read_jsonl(reflection_path)
        reflection_query = [r['reflection'] for r in reflection_results]
        ...
        reflection_retrieval_results = self.retriever.batch_search(reflection_query)
        ...
        input_prompts = [
            self.get_prompt(question, retrieval_result, reflection_retrieval_result, reflection_result, 
                            min_naive_confidence, self.retriever.topk, augumented_num)
